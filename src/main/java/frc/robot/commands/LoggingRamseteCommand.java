 ...
 */
public class LoggingRamseteCommand extends CommandBase {
  ...
  public Trajectory getTrajectory() {return m_trajectory;}
  ...
  public LoggingRamseteCommand(DriveTrain drivetrain, Pose2d start, List<Translation2d> interiorWaypoints, Pose2d end, boolean isReversed) {
    ...
    var centripitalAccelerationConstraint = new CentripetalAccelerationConstraint(2);
    ...
    m_trajectory = requireNonNullParam(exampleTrajectory, "trajectory", "RamseteCommand");
    m_pose = requireNonNullParam(drivetrain::getPose, "pose", "RamseteCommand");
    m_follower = requireNonNullParam(new RamseteController(), "follower", "RamseteCommand");
    m_kinematics = requireNonNullParam(Constants.DriveTrain.kDriveKinematics, "kinematics", "RamseteCommand");
    m_output =
        requireNonNullParam(drivetrain::setDriveMotorVoltage, "outputMetersPerSecond", "RamseteCommand");

    m_feedforward = Constants.DriveTrain.DRIVE_LINEAR_FF;
    m_speeds = drivetrain::getWheelSpeeds;
    m_leftController = new PIDController(0, 0, 0);
    m_rightController = new PIDController(0, 0, 0);
  ...
  public void execute() {
    ...
    m_prevTime = curTime;
    SmartDashboard.putNumber("Left Wheel Setpoint", leftSpeedSetpoint);
    SmartDashboard.putNumber("Right Wheel Setpoint", rightSpeedSetpoint);
    SmartDashboard.putNumber("Left Wheel Speed", m_speeds.get().leftMetersPerSecond);
    SmartDashboard.putNumber("Right Wheel Speed", m_speeds.get().rightMetersPerSecond);
  }
